...
declare -A CENTOS_VMs
CENTOS_VMs=( ["centos-node1"]="192.168.56.31" \
         ["centos-node2"]="192.168.56.32" \
         ["centos-node3"]="192.168.56.33" )
VM="VirtualBox"
DEPLOYED_KUBE=( "centos8_v1.22.4_calico" )
...
deploying_kube(){
    ...
    CONFIG_FILE=inventory/mycluster/hosts.yaml python3 contrib/inventory_builder/inventory.py "${CENTOS_VMs[@]}"
...
kubeUp(){
    ...
    if [ "$BOX_OS" != "centos8" ] ; then
        echo "[ERROR] Need env BOX_OS=centos8"
        exit 1
    fi
...
case "$1" in
    up)
        #export BOX_OS=centos8 KUBE_VERSION=v1.22.4 KUBE_NETWORK=calico
        kubeUp "$KUBE_VERSION" "$KUBE_NETWORK"
        ;;
    clean)
        kubeClean
        ;;
    ...
    *)
        ...
        echo 
        echo "Prerequisite:"
        echo "export BOX_OS=centos8 KUBE_VERSION=v1.22.4 KUBE_NETWORK=calico"
        echo
        ;;
esac
